...
func RunXORExperiment(seed int64) {
	...
	context := XORExperiment(seed, 3, true, false)
	...
	pointsContext := make(plotter.XYs, 0, len(context.Costs))
	for i, cost := range context.Costs {
		pointsContext = append(pointsContext, plotter.XY{X: float64(i), Y: float64(cost)})
	}
	...
	scatter, err = plotter.NewScatter(pointsContext)
	if err != nil {
		panic(err)
	}
	scatter.GlyphStyle.Radius = vg.Length(1)
